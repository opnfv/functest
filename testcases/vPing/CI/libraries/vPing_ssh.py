...
def main():
    ...
    while timeout > 0:
        ...
        if "Sending discover" in console_log and discover_count % 4 == 0:
            discover_count += 1 # to not print this message so often
            logger.debug("Console-log '%s': Sending discover..." % NAME_VM_2)
        elif "No lease, failing" in console_log and\
                "network failed" in console_log and not nolease_count:
                nolease_count = True
