...
def create_tempest_resources():
    ...
    if not tenant_id:
        logger.error("Error : Failed to create %s tenant" % CONF.tempest_tenant_name)
    ...
    user_id = os_utils.create_user(keystone_client,
                                   ...
                                   tenant_id)
    if not user_id:
        logger.error("Error : Failed to create %s user" % CONF.tempest_username)

    logger.debug("Creating private network for Tempest suite")
    network_dic = os_utils.create_shared_network_full(CONF.tempest_private_net_name,
