...
#
# from functest.opnfv_tests.openstack.snaps-oo import (
...
class HeatIms(clearwater_ims_base.ClearwaterOnBoardingBase):
    ...
    def deploy_vnf(self):
        ...
        cw_heat_client.deployments.create(descriptor.get('name'),
                                          descriptor.get('name'),
                                          self.vnf.get('inputs'))
        get_exec_env = _get_deployment_environment_creation_execution(
            cw_heat_client, descriptor.get('name'))
        wait_for_execution(cw_heat_client, get_exec_env, self.__logger,
                           timeout=600)
        ...
        execution = cw_heat_client.executions.start(descriptor.get('name'),
                                                    'install')
        # Show execution log
        execution = wait_for_execution(cw_heat_client, execution,
                                       self.__logger)
    ...
    def clean(self):
        ...
        try:
            ...
            cw_heat_client.deployments.delete(
                self.vnf['descriptor'].get('name'))
            cw_heat_client.blueprints.delete(
                self.vnf['descriptor'].get('name'))
        except BaseException:  # pylint: disable=broad-except
