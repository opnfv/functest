...
def create_flavor(nova_client, flavor_name, ram, disk, vcpus):
    try:
        ...
        try:
            extra_specs = ft_utils.get_functest_config('general.flavor_extra_specs')
        ...
        if image_id != '':
            ...
            if logger:
                logger.info("Creating image '%s' from '%s'..." % (image_name,
                                                                  file_path))
            try:
                properties = ft_utils.get_functest_config('general.image_properties')
            except ValueError:
                # image properties are not configured, therefore don't add any properties
                properties = {}
