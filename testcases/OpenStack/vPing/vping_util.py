...
def boot_vm(case, vmname, image_id, flavor, network_id, test_ip, sg_id, logger):
    EXIT_CODE = -1
    ...
    add_secgroup(vmname, vm.id, sg_id, logger)
...
def establish_ssh(vm, floatip, logger):
    EXIT_CODE = -1
...
def transfer_ping_script(ssh, floatip, logger):
    EXIT_CODE = -1
...
def do_vping_ssh(ssh, test_ip, logger):
...
def do_vping(case, vm, test_ip, logger):
    if is_userdata(case):
        ...
        floatip = add_float_ip(vm, logger)
        ssh = establish_ssh(vm, floatip, logger)
        transfer_ping_script(ssh, floatip, logger)
        return do_vping_ssh(ssh, test_ip, logger)
...
def push_result(report, case, logger, start_time, stop_time, details):
    if report:
        try:
