...
following docker command::
  ...
  opnfv/functest colorado.3.0    13fa54a1b238  4 weeks ago  1.29 MB
  opnfv/functest colorado.1.0    94b78faa94f7  9 weeks ago  968 MB
  hello-world    latest          94df4f0ce8a4  10 weeks ago    967 B
...
recommended parameters for invoking docker container
  ...
  #. Passing deployment scenario
     ...
     **NOTE:** The scenario name is mainly used to automatically detect
     ...
     to be run on ODL scenarios). If not set, Functest will try to run
     incompatible test suites on the SUT.

     **NOTE:** HA mode must be considered from an OpenStack perspective. A HA
     scenario does not mean that all the components of the SUT are HA. SDN
     controllers for instance are still non HA.
