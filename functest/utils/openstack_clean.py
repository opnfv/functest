...
import functest.utils.openstac_tacker as os_tacker
import yaml
...
def remove_vnfds(tacker_client, default_vnfds):
    ...
    for vnfd in vnfds:
        vnfd_name = getattr(vnfd, 'name')
        vnfd_id = getattr(vnfd, 'id')
...
def remove_vnfs(tacker_client, default_vnfs):
    ...
    for vnf in vnfs:
        vnf_name = getattr(vnf, 'name')
        vnf_id = getattr(vnf, 'id')
...
def remove_sfcs(tacker_client, default_sfcs):
    ...
    for sfc in sfcs:
        sfc_name = getattr(sfc, 'name')
        sfc_id = getattr(sfc, 'id')
...
def remove_sfc_classifiers(tacker_client, default_sfc_classifiers):
    ...
    sfc_clfs = os_tacker.list_sfc_classifiers(tacker_client, verbose=True)
    if sfc_clfs is None:
        logger.debug("There are no SFC classifiers in the deployment. ")
        return

    for sfc_clf in sfc_clfs:
        sfc_clf_name = getattr(sfc_clf, 'name')
        sfc_clf_id = getattr(sfc_clf, 'id')
