...
function run_test(){
    ...
    case $test_name in
        ...
        "snaps-all")
            ...
            if [ ! -d $results_dir/snaps ]; then
                mkdir $results_dir/snaps
            fi;
            python $repos_dir/snaps/snaps/unit_test_suite.py -e $creds -n $ext_net -k -p 10.197.123.27:3128 &> $results_dir/snaps/all-tests-log.out
        ;;
        "snaps-no-fip")
            ext_net=$(cat ${CONFIG_FUNCTEST_YAML} | shyaml get-value snaps.external_network_name 2> /dev/null || true)
            results_dir=$(cat ${CONFIG_FUNCTEST_YAML} | shyaml get-value general.directories.dir_results 2> /dev/null || true)
            ...
            python $repos_dir/snaps/snaps/unit_test_suite.py -e $creds -n $ext_net -k -f -p 10.197.123.27:3128 &> $results_dir/snaps/test-no-fip-log.out
        ;;
        "vping_ssh")
