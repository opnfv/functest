...
class Sfc_fun:
    ...
    def checkVmState(self):
        ...
        time.sleep(10)
        for y in range(0, 3):
            url = 'http://'+self.os_hostname+':8774/v2.1/servers/\
                  detail?name=vm'+str(y)
            headers = {"Accept": "application/json",  "X-Auth-Token\
                       ": self.token_id}
            response = requests.get(url, headers=headers)
            if (response.status_code == 200):
    ...
    def createPortPair(self):
        ...
        for p in range(1, 2):
            ...
            url = 'http://'+self.os_hostname+':9696/'+self.osver+'/sfc/\
                  port_pairs'
            headers = {"Accept": "application/json", "X-Auth-Token\
                ": self.token_id}
            response = requests.post(url, headers=headers,  data=data)
    ...
    def getPortGroup(self):
        ...
        for p in range(0, 1):
            url = 'http://'+self.os_hostname+':9696/'+self.osver+'/sfc/\
                port_pair_groups?name=PG'+str(p)
            headers = {"Accept": "application/json", "X-Auth-Token\
                       ": self.token_id}
            response = requests.get(url, headers=headers)
            ...
            if (response.status_code == 200):
                ...
                self.logger.debug(response.content)
                json1_data = json.loads(response.content)
                self.port_grp_id.append(json1_data['\
                     port_pair_groups'][0]['id'])
            else:
                return(response.status_code)
        return(response.status_code)
    ...
    def checkFlowAdded(self):
        ...
        time.sleep(5)
        response = requests.get('http://'+self.onos_hostname+':8181/onos/v1/\
                   flows',  auth=("karaf",  "karaf"))
        if (response.status_code == 200):
...
####################################################################
    ...
    def attachInterface(self):
        ...
        data = json.dumps(Dicdata, indent=4)
        url = 'http://'+self.os_hostname+':9696/'+self.osver+'/routers/\
               '+self.router_id+'/add_router_interface'
        headers = {"Accept": "application/json",  "\
                   X-Auth-Token": self.token_id}
    ...
    def portChainDeviceMap(self):
        ...
        response = requests.get('http://'+self.onos_hostname+':8181/\
               onos/vtn/portChainDeviceMap/'+self.PC_id,  auth=("\
                karaf",  "karaf"))
        if (response.status_code == 200):
    ...
    def portChainSfMap(self):
        ...
        response = requests.get('http://'+self.onos_hostname+':8181/onos/\
                   vtn/portChainSfMap/'+self.PC_id,  auth=("karaf",  "karaf"))
        if (response.status_code == 200):
    ...
    def deleteFlowClassifier(self):
        ...
        url = 'http://'+self.os_hostname+':9696/'+self.osver+'/sfc/\
               flow_classifiers/'+self.flow_class_if
        headers = {"Accept": "application/json",  "\
                   X-Auth-Token": self.token_id}
        response = requests.delete(url, headers=headers)
        if (response.status_code == 200):
    ...
    def deletePortGroup(self):
        ...
        for p in range(0, 1):
            url = 'http://'+self.os_hostname+':9696/'+self.osver+'/sfc/\
                   port_pair_groups/'+self.port_grp_id[p]
            headers = {"Accept": "application/json", "X-Auth-Token\
                      ": self.token_id}
            response = requests.delete(url, headers=headers)
    ...
    def deletePortPair(self):
        ...
        for p in range(1,  2):
            url = 'http://'+self.os_hostname+':9696/'+self.osver+'/sfc/\
                   port_pairs/'+self.port_pair_id[0]
            headers = {"Accept": "application/json", "\
                       X-Auth-Token": self.token_id}
            response = requests.delete(url, headers=headers)
            if (response.status_code == 204):
    ...
    def cleanup(self):
        ...
        if (response.status_code == 200):
            ...
            data = json.dumps(Dicdata1, indent=4)
            url = 'http://'+self.os_hostname+':9696/'+self.osver+'/routers/\
                  '+self.router_id+'/remove_router_interface.json'
            headers = {"Accept": "application/json", "\
                       X-Auth-Token": self.token_id}
            response = requests.put(url, headers=headers,  data=data)
            if (response.status_code == 200):
