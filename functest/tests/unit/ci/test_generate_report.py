...
class GenerateReportTesting(unittest.TestCase):
    ...
    def test_print_line_with_ci_run(self):
        ...
        w4 = ''
        w5 = ''
        str = ('| ' + w1.ljust(gen_report.COL_1_LEN - 1) +
               '| ' + w2.ljust(gen_report.COL_2_LEN - 1) +
               '| ' + w3.ljust(gen_report.COL_3_LEN - 1) +
               '| ' + w4.ljust(gen_report.COL_4_LEN - 1) +
               '| ' + w5.ljust(gen_report.COL_5_LEN - 1))
        str += '|\n'
        self.assertEqual(gen_report.print_line(w1), str)
    ...
    def test_print_line_no_column_with_ci_run(self):
        ...
        self.assertEqual(gen_report.print_line_no_columns('test'), str)
