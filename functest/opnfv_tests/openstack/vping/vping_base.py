...
class VPingBase(TestCasesBase.TestCasesBase):
    """
    Base class for vPing tests that check connectivity between two VMs shared internal network.
    ...
    def __init__(self):
        ...
        self.os_creds = openstack_tests.get_credentials(os_env_file=get_conf('general.openstack.creds'))
        ...
        self.private_net_name = get_conf('vping.vping_private_net_name') + self.guid
        self.private_subnet_name = get_conf('vping.vping_private_subnet_name') + self.guid
    ...
    def run(self):
        ...
        if not os.path.exists(self.functest_repo):
            raise Exception("Functest repository not found '%s'" % self.functest_repo)
        ...
        self.logger.info("Creating network with name: '%s'" % self.private_net_name)
        self.network_creator = deploy_utils.create_network(
            self.os_creds, NetworkSettings(self.os_creds, name=self.private_net_name,
                                           subnet_settings=[SubnetSettings(self.os_creds,
    ...
    def _execute(self):
        ...
        test_ip = self.vm1_creator.get_port_ip(self.vm1_creator.instance_settings.port_settings[0].name)

        if self.vm1_creator.vm_active(block=True) and self.vm2_creator.vm_active(block=True):
    ...
    def _cleanup(self):
        ...
        if self.self_cleanup:
            if self.vm2_creator:
                self.logger.info("Deleting VM 2 instance with name: '%s'" % self.vm2_creator.instance_settings.name)
                self.vm2_creator.clean()
            if self.vm1_creator:
                self.logger.info("Deleting VM 1 instance with name: '%s'" % self.vm1_creator.instance_settings.name)
                self.vm1_creator.clean()
            if self.router_creator:
                self.logger.info("Deleting router instance with name: '%s'" % self.router_creator.router_settings.name)
                self.router_creator.clean()
            if self.kp_creator:
                self.logger.info("Deleting keypair with name: '%s'" % self.kp_creator.keypair_settings.name)
                self.kp_creator.clean()
            if self.sg_creator:
                self.logger.info("Deleting security group with name: '%s'" % self.sg_creator.sec_grp_settings.name)
                self.sg_creator.clean()
            if self.network_creator:
                self.logger.info("Deleting network with name: '%s'" % self.network_creator.network_settings.name)
                self.network_creator.clean()
            if self.image_creator:
                self.logger.info("Deleting image with name: '%s'" % self.image_creator.image_settings.name)
