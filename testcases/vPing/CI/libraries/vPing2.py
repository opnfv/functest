...
def main():
    ...
    floatip = functest_utils.create_floating_ip(nova_client)
    if floatip == None:
    ...
    logger.info("SCP ping scrip to VM2...")
    cmd = "sshpass -p 'cubswin:)' scp ping.sh cirros@"+floatip+":~/test.sh"
    functest_utils.execute_command(cmd, logger)
    
    cmd = "sshpass -p 'cubswin:)' ssh cirros@"+floatip+" '~/ping.sh +"+IP_1+"'"
    functest_utils.execute_command(cmd, logger)
