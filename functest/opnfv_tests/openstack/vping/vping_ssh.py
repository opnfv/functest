...
class VPingSSH(vping_base.VPingBase):
    ...
    def __init__(self):
        ...
        self.kp_name = \
            functest_utils.get_functest_config('vping.keypair_name') + \
            self.guid
        self.kp_priv_file = \
            functest_utils.get_functest_config('vping.keypair_priv_file')
        self.kp_pub_file = \
            functest_utils.get_functest_config('vping.keypair_pub_file')
        self.router_name = \
            functest_utils.get_functest_config('vping.vping_router_name') + \
            self.guid
        self.sg_name = \
            functest_utils.get_functest_config('vping.vping_sg_name') + \
            self.guid
        self.sg_desc = \
            functest_utils.get_functest_config('vping.vping_sg_descr')

        neutron = neutron_utils.neutron_client(self.os_creds)
        ext_nets = neutron_utils.get_external_networks(neutron)
        # take first
        self.ext_net = ext_nets[0]
        ...
        try:
            ...
            self.router_creator = deploy_utils.create_router(
                ...
                RouterSettings(
                    ...
                    external_gateway=self.ext_net['network']['name'],
    ...
    def __create_security_group(self):
        ...
        sg_rules = list()
        sg_rules.append(
            SecurityGroupRuleSettings(self.os_creds, sec_grp_name=self.sg_name,
