...
def configure_tempest_feature(deployment_dir, mode):
    ...
    if mode == 'feature_multisite':
        ...
        cmd= "openstack endpoint show kingbird | grep publicurl | awk '{print $4}' | awk -F '/' '{print $3}'"
        kingbird_endpoint_url = os.popen(cmd).read()
        cmd = "openstack endpoint show kingbird | grep publicurl | awk '{print $4}' | awk -F '/' '{print $4}'"
