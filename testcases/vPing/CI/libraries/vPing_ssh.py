...
def cleanup(nova, neutron, image_id, network_dic, sg_id, floatingip):
    ...
    if not functest_utils.delete_floating_ip(nova_client, floatingip['fip_id']):
        ...
        for line in output:
            if "vPing OK" in line:
                ...
                break
                    ...
                    "network failed" in console_log:
                logger.info("It seems the instance failed to get the ip from "\
                            "the DHCP agent. msg: 'No lease, failing.'"\
