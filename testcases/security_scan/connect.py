...
class novaManager:
    ...
    def keystonepass(self):
        ...
        try:
            ...
            logger.error("Password is invalid for undercloud host: {0}").format(INSTALLER_IP)
        except paramiko.AuthenticationException:
            print "Authentication failed"
            logger.error ("Authentication failed for undercloud host: {0}").format(INSTALLER_IP)
        except socket.error, e:
            print "Socket connection failed"
            logger.error("Socker Connection failed for undercloud host: {0}").format(INSTALLER_IP)
...
class connectionManager:
    ...
    def remotescript(self):
        ...
        try:
            ...
            print "Password is invalid:" , e
            logger.error ("Authentication failed for host: {0}").format(host)
        except paramiko.AuthenticationException:
            print "Authentication failed"
            logger.error ("Authentication failed for host: {0}").format(host)
        except socket.error, e:
            print "Socket connection failed"
            logger.error("Socker Connection failed for undercloud host: {0}").format(host)

        transport = client.get_transport()
        local_addr = ('127.0.0.1', 0)
        channel = transport.open_channel("direct-tcpip", (self.host, int(self.port)), (local_addr))
        remote_client = paramiko.SSHClient()
        remote_client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        # Tunnel to overcloud
        try:
            remote_client.connect('127.0.0.1', port=22, username=self.user, key_filename=self.user_key, sock=channel)
