...
class ODLTesting(unittest.TestCase):
    ...
    def test_main_set_robotframework_vars_failed(self):
        with mock.patch.object(self.test, 'set_robotframework_vars',
                               return_value=False):
            self._test_main(TestcaseBase.EX_RUN_ERROR)
    ...
    def test_main_robot_run_failed(self, *args):
        with mock.patch.object(self.test, 'set_robotframework_vars',
            ...
            self._test_main(TestcaseBase.EX_RUN_ERROR, *args)
    ...
    def test_main_remove_exception(self, *args):
        with mock.patch.object(self.test, 'set_robotframework_vars',
                ...
                mock.patch.object(self.test, 'parse_results'), \
                self.assertRaises(Exception):
            self._test_main(TestcaseBase.EX_OK, *args)
    ...
    def _test_run(self, status=TestcaseBase.EX_OK,
    ...
    def test_run_main_false(self):
        ...
        self._test_run(TestcaseBase.EX_RUN_ERROR,
                       odlip=self._sdn_controller_ip,
                       odlwebport=self._odl_webport)
    ...
    def test_run_main_exception(self):
        ...
        with self.assertRaises(Exception):
            self._test_run(status=TestcaseBase.EX_RUN_ERROR,
                           exception=Exception(),
                           odlip=self._sdn_controller_ip,
                           odlwebport=self._odl_webport)
    ...
    def test_run_without_installer_type(self):
        ft_constants.SDN_CONTROLLER_IP = self._sdn_controller_ip
        ft_constants.CI_INSTALLER_TYPE = None
        self._test_run(TestcaseBase.EX_OK,
                       odlip=self._sdn_controller_ip,
                       odlwebport=self._odl_webport)
