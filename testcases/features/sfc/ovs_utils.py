...
class OVSLogger(object):
    ...
    def __dump_to_file(self, operation, host, text):
        self.__mkdir_p(host)
        timestamp = time.strftime("%Y%m%d-%H%M%S")
        fname = '_'.join(operation, timestamp)
        with open(os.path.join(self.ovs_dir, fname), 'w') as f:
            f.write(text)
    ...
    def ofctl_dump_flows(self, ssh_conn, br='br-int', choose_table=None):
        try:
            ...
            host = self.__ssh_host(ssh_conn)
            self.__dump_to_file(operation, host, output)
            return output
        except Exception, e:
