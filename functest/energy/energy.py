...
"""This module manage calls to Energy recording API."""
...
def set_step(step):
    """
    Decorator to defined intermediary step in running testCase.

        param step: Step to set
        :type step: string
    """
    def method_decorator_wrapper(method):
        """
        Decorator to defined intermediary step in running testCase.

            param method: Method for with step is defined
            :type method: function
        """
        def final_wrapper(*args):
            """Wrapper for decorator to handle method arguments."""
            EnergyRecorder.set_step(step)
            method(*args)
        return final_wrapper
    return method_decorator_wrapper
...
class EnergyRecorder(object):
    ...
    def load_config():
        """
        Load conenctivity settings from yaml.
    ...
    def start(scenario):
        ...
        try:
            ...
            # is safe for caller
            EnergyRecorder.logger.exception("Error while starting energy recorder API")
            return_status = False
    ...
    def stop():
        ...
        try:
            ...
            # Default exception handler to ensure that method
            # is safe for caller
            EnergyRecorder.logger.exception("Error while stoping energy recorder API")
            return_status = False
        return return_status
    ...
    def set_step(step):
        ...
        try:
            ...
            # Default exception handler to ensure that method
            # is safe for caller
            EnergyRecorder.logger.exception("Error while setting step on energy recorder API")
            return_status = False
        return return_status
