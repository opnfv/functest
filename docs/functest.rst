...
A script as been added to automatically retrieve the credentials.
Howeber, if you still install manually functest, you will need to source the rc file on the machine you are running the tests.
...
In SR1, we added some code in order to push the results (status and duration) into a centralized test result database.
...
When integrated in CI, there is no additional prerequisites.
 ...
 * installer_ip the IP of your jumphost
