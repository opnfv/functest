...
class StorageValidation(testcase.TestCase):  # pylint: disable=too-few-public-methods
    '''
    ...
    def run(self):
        ...
        '''
        commands = pkg_resources.resource_filename('functest', 'opnfv_tests/rbdbench/commands.yaml')
        conf_cli = yaml.safe_load(open(commands, 'r'))
        rbd_command = conf_cli.split(",")
        client = paramiko.SSHClient()
        cred = pkg_resources.resource_filename('functest', 'opnfv_tests/rbdbench/cred.yaml')
        ...
        for command in rbd_command:
            stdin, stdout, stderr = client.exec_command(command) #pylint: disable=unused-variable
