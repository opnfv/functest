...
class Sctp(singlevm.SingleVm2):
    ...
    def __init__(self, **kwargs):
        ...
        super(Sctp, self).__init__(**kwargs)
    ...
    def prepare(self):
        ...
        #self.port2sshvm = self.cloud.create_port(network_id=self.network2.id, security_groups=[self.sec.id])
        #self.port2vm2 = self.cloud.create_port(network_id=self.network2.id,security_groups=[self.sec.id])
        #self.cloud.attach_port(self.sshvm, self.port2sshvm)
        #self.cloud.attach_port(self.vm2, self.port2vm2)
    ...
    def _do_sctp_darn(self):
        ...
        Override from super
        """
        if not (self.vm2.private_v4 or self.vm2.addresses[
        ...
        self.logger.info("vm2 net1:{} sshvm net1:{} vm2 net2:{} sshvm net2: {}".format(
        ...
        self.logger.info(
            "lksctp-tools install server stdout: %s", stdout.read().decode("utf-8"))
        self.logger.info(
            "lksctp-tools install server stderr: %s", stderr.read().decode("utf-8"))
        ...
        (_, stdout, stderr) = self.ssh2.exec_command(
            "sudo ip link set dev eth1 up && \
             ...
             sudo apt-get install -y lksctp-tools")
        self.logger.info(
            "lksctp-tools install client stdout: %s", stdout.read().decode("utf-8"))
        self.logger.info(
            "lksctp-tools install client stderr: %s", stderr.read().decode("utf-8"))

        self.logger.debug("ssh: %s", self.ssh)
        (_, stdout, stderr) = self.ssh.exec_command(
            "sudo sctp_darn -l -P 5201 -H %s -B %s > /tmp/sctp_darn_logs 2>&1 &" % (
             self.sshvm.addresses[self.network.name][1].addr,
             self.sshvm.addresses[self.network.name][0].addr))
        self.logger.info(
        ...
        self.logger.debug("ssh: %s", self.ssh2)
        (_, stdout, stderr) = self.ssh2.exec_command(
            "(i=1; while [ $i -le 20 ]; do echo blah; sleep 1; ((i++));  done)|sudo sctp_darn -P 5201 -H %s -h %s -p 5201 -B %s -s" % (
              self.vm2.addresses[self.network.name][1].addr,
              self.sshvm.addresses[self.network.name][1].addr,
              self.vm2.addresses[self.network.name][0].addr))
