...
def run_cmd_on_fm(cmd, ip="10.20.0.2", username="root", passwd="r00tme"):
    ssh_cmd = "sshpass -p %s ssh %s %s@%s %s" % (passwd, ssh_options, username, ip, cmd)
...
def run_cmd_remote(ip, cmd, username="root", passwd="opnfv"):
    ...
    ssh_cmd = "sshpass -p %s ssh %s %s@%s %s" % (passwd, ssh_opt_append, username, ip, cmd)
    return run_cmd(ssh_cmd)
...
def main():
    ...
    if len(eval(env_list)) > 1 and fuel_env is None:
        logger.error('\033[91mMore than one fuel env found, export FUEL_ENV=<env-od> to set ENV\033[0m\n %s' % run_cmd_on_fm("fuel env"))
        sys.exit(1)
    ...
    server_ip, client_ip, sf1, sf2 = get_floating_ips( nova_client, neutron_client)
    ...
    if not start_http_server(server_ip):
        logger.error('\033[91mFailed to start HTTP server on %s\033[0m' % server_ip)
        sys.exit(1)
    ...
    if json_results["failures"]:
        status = "FAIL"
        logger.error('\033[91mSFC TESTS: %s :( FOUND %s FAIL \033[0m' % (status, json_results["failures"]))
