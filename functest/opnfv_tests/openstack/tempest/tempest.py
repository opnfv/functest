...
class TempestCommon(TestCasesBase.TestCasesBase):

    def __init__(self):
        self.GlobalVariables.CASE_NAME = ""
    ...
    def configure_tempest(self):
        ...
        tempest_conf_file = self.deployment_dir + "/tempest.conf"
    ...
    def generate_test_list(self):
        ...
        if GlobalVariables.MODE == 'defcore':
            shutil.copyfile(TEMPEST_DEFCORE, TEMPEST_RAW_LIST)
        elif GlobalVariables.MODE == 'custom':
            ...
            if GlobalVariables.MODE == 'smoke':
                testr_mode = "smoke"
            elif GlobalVariables.MODE == 'feature_multisite':
                testr_mode = " | grep -i kingbird "
            elif GlobalVariables.MODE == 'full':
                testr_mode = ""
            else:
                testr_mode = 'tempest.api.' + GlobalVariables.MODE
            cmd = ("cd " + self.deployment_dir + ";" + "testr list-tests " +
                   testr_mode + ">" + TEMPEST_RAW_LIST + ";cd")
            ft_utils.execute_command(cmd)

    def apply_tempest_blacklist(self):
        logger.debug("Applying tempest blacklist...")
        cases_file = self.read_file(TEMPEST_RAW_LIST)
    ...
    def run(self, OPTION):
        ...
        if 'smoke' in GlobalVariables.MODE:
            GlobalVariables.CASE_NAME = 'tempest_smoke_serial'
        elif 'feature' in GlobalVariables.MODE:
            GlobalVariables.CASE_NAME = GlobalVariables.MODE.replace(
                "feature_", "")
        else:
            GlobalVariables.CASE_NAME = 'tempest_full_parallel'

        status = ft_utils.check_success_rate(
            GlobalVariables.CASE_NAME, success_rate)
        logger.info("Tempest %s success_rate is %s%%, is marked as %s"
                    % (GlobalVariables.CASE_NAME, success_rate, status))
        ...
        if args.report:
            ...
            json_results = {"timestart": time_start,
...
class TempestSmoke(TempestCommon):
    ...
    def __init__(self):
        ...
        GlobalVariables.CASE_NAME = "tempest_smoke_serial"
        super(TempestSmoke, self).generate_test_list()
...
class TempestMultisite(TempestCommon):
    ...
    def __init__(self):
        GlobalVariables.CASE_NAME = "multisite"
        GlobalVariables.MODE += " --concur 1"
...
def main():
    ...
    if not (GlobalVariables.MODE in modes):
    ...
    ret_val = run(GlobalVariables.MODE)
    if ret_val != 0:
        sys.exit(-1)
