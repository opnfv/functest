...
class CheckDeployment:
    ...
    @staticmethod
    def check_rc(rc_file):
        if not os.path.isfile(rc_file):
            raise Exception('RC file {} does not exist!'.format(rc_file))

        if 'OS_AUTH_URL' not in open(rc_file).read():
            raise Exception('OS_AUTH_URL not defined in {}.'.format(rc_file))
        _print_ok()

    @staticmethod
    def check_auth_endpoint(os_creds):
    ...
    @staticmethod
    def check_public_endpoint(os_creds):
    ...
    @staticmethod
    def check_service_endpoint(service, os_creds):
