...
info ()  {
    echo -e "$(date) - healtcheck.INFO:" "$*" | tee -a $LOG_FILE 1>&2
}

debug ()  {
    if [[ "${CI_DEBUG,,}" == "true" ]]; then
        echo -e "$(date) - healtcheck.DEBUG:" "$*" | tee -a $LOG_FILE 1>&2
    fi
}

error () {
    echo -e "$(date) - healtcheck.ERROR:" "$*" | tee -a $LOG_FILE 1>&2
    exit 1
}
