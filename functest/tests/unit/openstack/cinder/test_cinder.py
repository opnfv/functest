...
class CinderTesting(unittest.TestCase):
    ...
    def test_execute_write(self):
        ...
        stdout = mock.Mock()
        stderr = mock.Mock()
        vol_dev_name = mock.Mock()
        self.cinder.ssh.exec_command.return_value = (None, stdout, stderr)
        with mock.patch.object(self.cinder, '_read_data',
                               return_value=False), \
            mock.patch.object(self.cinder.cloud, 'detach_volume',
                              return_value=True):
            self.cinder.execute()
            self.cinder.ssh.exec_command("sh ~/write_data.sh {}".format(
                vol_dev_name))
            self.cinder.ssh.exec_command.assert_called_once_with(
                "sh ~/write_data.sh {}".format(vol_dev_name))
    ...
    def test_execute_read(self):
        ...
        stderr = mock.Mock()
        vol_dev_name = mock.Mock()
        self.cinder.ssh2.exec_command.return_value = (None, stdout, stderr)
        with mock.patch.object(self.cinder, '_write_data',
                               return_value=False), \
            mock.patch.object(self.cinder.cloud, 'detach_volume',
                              return_value=True):
            self.cinder.execute()
            self.cinder.ssh2.exec_command("sh ~/read_data.sh {}".format(
                vol_dev_name))
            self.cinder.ssh2.exec_command.assert_called_once_with(
                "sh ~/read_data.sh {}".format(vol_dev_name))
