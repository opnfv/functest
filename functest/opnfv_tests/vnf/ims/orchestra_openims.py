...
class OpenImsVnf(vnf.VnfOnBoarding):
    ...
    def __init__(self, **kwargs):
        ...
        self.orchestrator = dict(
            get_config("orchestrator", config_file),
            details={}
        )
        ...
        self.images = get_config("tenant_images.%s" % self.case_name, config_file)
        self.images.update(get_config("tenant_images.%s" % self.case_name, config_file))
    ...
    def prepare_security_groups(self):
        ...
        self.orchestrator['details']['sec_group'] = security_group_info.name
        self.logger.info(
    ...
    def prepare_flavor(self):
        ...
        flavor_settings = FlavorSettings(
            ...
            disk=self.orchestrator['requirements']['flavor']['disk'],
            vcpus=self.orchestrator['requirements']['flavor']['vcpus'])
        flavor = OpenStackFlavor(self.snaps_creds, flavor_settings)
        flavor_info = flavor.create()
        self.created_resources.append(flavor)
        self.orchestrator['details']['flavor'] = {}
        self.orchestrator['details']['flavor']['name'] = flavor_settings.name
        self.orchestrator['details']['flavor']['id'] = flavor_info.id
    ...
    def prepare_network(self):
        ...
        self.orchestrator['details']['network'] = {}
        self.orchestrator['details']['network']['id'] = orchestra_network_info.id
        self.orchestrator['details']['network']['name'] = orchestra_network_info.name
        self.orchestrator['details']['external_net_name'] = snaps_utils.get_ext_net_name(
            self.snaps_creds)
        ...
        orchestra_router = OpenStackRouter(
            ...
            RouterSettings(
                name='%s_router' %
                self.case_name,
                external_gateway=self.orchestrator['details']['external_net_name'],
                internal_subnets=[
            ...
            for my_floating_ip in my_floating_ips:
                for snaps_floating_ip in snaps_floating_ips:
                    if snaps_floating_ip.ip == my_floating_ip:
                        self.orchestrator['details']['fip'] = snaps_floating_ip
                        self.logger.info(
                            "Selected floating IP for Open Baton NFVO %s",
                            (self.orchestrator['details']['fip'].ip))
                        break
                if self.orchestrator['details']['fip'] is not None:
                    break
        else:
            self.logger.info("Creating floating IP for Open Baton NFVO")
            self.orchestrator['details']['fip'] = snaps_utils.neutron_utils.create_floating_ip(
                neutron_client, self.orchestrator['details']['external_net_name'])
            self.logger.info(
    ...
    def get_vim_descriptor(self):
        ...
        vim_json = {
            ...
            "securityGroups": [
                self.orchestrator['details']['sec_group']
            ],
            "type": "openstack",
            "location": {
                "name": "opnfv",
    ...
    def deploy_orchestrator(self):
        ...
        self.logger.info("Details: %s", self.orchestrator['details'])
        start_time = time.time()

        self.logger.info("Creating orchestra instance...")
        userdata = get_userdata(self.orchestrator)
        self.logger.info("flavor: %s\n"
                         "image: %s\n"
                         "network_id: %s\n",
                         self.orchestrator['details']['flavor']['name'],
                         self.orchestrator['requirements']['image'],
                         self.orchestrator['details']['network']['id'])
        self.logger.debug("userdata: %s\n", userdata)
        # setting up image
        image_settings = ImageSettings(
            name=self.orchestrator['requirements']['image'],
            image_user='ubuntu',
            exists=True)
        # setting up port
        port_settings = PortSettings(
            name='%s_port' % self.case_name,
            network_name=self.orchestrator['details']['network']['name'])
        # build configuration of vm
        orchestra_settings = VmInstanceSettings(
            name=self.case_name,
            flavor=self.orchestrator['details']['flavor']['name'],
            port_settings=[port_settings],
            security_group_names=[self.orchestrator['details']['sec_group']],
            userdata=userdata)
        ...
        self.orchestrator['details']['id'] = orchestra_vm.get_vm_info()['id']
        self.logger.info(
            "Created orchestra instance: %s",
            self.orchestrator['details']['id'])
        ...
        while timeout < 200:
            if servertest(
                ...
                self.logger.info("Open Baton NFVO is not started yet (%ss)", (timeout * 5))
                time.sleep(5)
                timeout += 1
    ...
    def deploy_vnf(self):
        ...
        main_agent = MainAgent(
            nfvo_ip=self.orchestrator['details']['fip'].ip,
            nfvo_port=8080,
            https=False,
            version=1,
            username=self.orchestrator['credentials']['username'],
            password=self.orchestrator['credentials']['password'])
        ...
        for project in json.loads(project_agent.find()):
            if project.get("name") == "default":
                self.orchestrator['details']['project_id'] = project.get("id")
        ...
        main_agent.get_agent(
            "vim", project_id=self.orchestrator['details']['project_id']).create(
                entity=json.dumps(vim_json))

        market_agent = main_agent.get_agent(
            "market", project_id=self.orchestrator['details']['project_id'])
        ...
        try:
            ...
            if nsd.get('id') is None:
                ...
                return False
            self.orchestrator['details']['nsd_id'] = nsd.get('id')
            self.logger.info("Onboarded NSD: " + nsd.get("name"))

            nsr_agent = main_agent.get_agent(
                "nsr", project_id=self.orchestrator['details']['project_id'])

            self.orchestrator['details']['nsr'] = nsr_agent.create(
                self.orchestrator['details']['nsd_id'])
        ...
        if self.orchestrator['details']['nsr'].get('code') is not None:
            self.logger.error(
                "%s cannot be deployed: %s -> %s",
                self.vnf['name'],
                self.orchestrator['details']['nsr'].get('code'),
                self.orchestrator['details']['nsr'].get('message'))
        ...
        while self.orchestrator['details']['nsr'].get("status") != 'ACTIVE' \
                and self.orchestrator['details']['nsr'].get("status") != 'ERROR':
            timeout += 1
            ...
            if timeout == 300:
                ...
                return False
            time.sleep(5)
            self.orchestrator['details']['nsr'] = json.loads(
                nsr_agent.find(self.orchestrator['details']['nsr'].get('id')))

        duration = time.time() - start_time
        if self.orchestrator['details']['nsr'].get("status") == 'ACTIVE':
            self.details["vnf"].update(status='PASS', duration=duration)
            self.logger.info("Sleep for 60s to ensure that all services are up and running...")
            time.sleep(60)
            result = True
        else:
            self.details["vnf"].update(status='FAIL', duration=duration)
            self.logger.error("NSR: %s", self.orchestrator['details'].get('nsr'))
            result = False
        return result
    ...
    def test_vnf(self):
        ...
        self.logger.info(
            ...
            self.orchestrator['details']['nsr'].get('name'))
        for vnfr in self.orchestrator['details']['nsr'].get('vnfr'):
            self.logger.info(
                ...
                for vnfci in vdu.get('vnfc_instance'):
                    ...
                    for floating_ip in vnfci.get('floatingIps'):
                        self.logger.debug(
                            ...
                            vnfci.get('hostname'),
                            floating_ip.get('ip'))
                        for port in self.vnf['test'][vnfr.get('name')]['ports']:
    ...
    def clean(self):
        ...
        try:
            ...
            if (self.orchestrator['details'].get('nsr')):
                main_agent.get_agent(
                    "nsr", project_id=self.orchestrator['details']['project_id']).delete(
                        self.orchestrator['details']['nsr'].get('id'))
            ...
            #                          instance_id=self.orchestrator_instance_id)
        ...
        try:
            ...
            snaps_utils.neutron_utils.delete_floating_ip(neutron_client,
                self.orchestrator['details']['fip'])
        except Exception as exc:
            self.logger.error('Unexpected error cleaning - %s', exc)
