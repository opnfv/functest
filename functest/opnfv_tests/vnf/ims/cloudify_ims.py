...
class CloudifyIms(clearwater_ims_base.ClearwaterOnBoardingBase):
        ...
        for image_name, image_url in self.images.iteritems():
            ...
            try:
                image_id = os_utils.get_image_id(self.glance_client,
                                                 image_name)
                self.__logger.debug("image_id: %s", image_id)
            except Exception:  # pylint: disable=broad-except
                self.__logger.error("Unexpected error: %s", sys.exc_info()[0])
                raise vnf.VnfPreparationException

            if image_id == '':
                self.__logger.info("""%s image does not exist on glance repo.
    ...
    def deploy_orchestrator(self):
        ...
        self.__logger.info("Creating floating IP for cloudify manager vm ...")
        ...
        self.exist_obj['flavor1'], flavor_id = os_utils.get_or_create_flavor(
            ...
            '2',
            public=True)
        self.__logger.debug("Flavor id: %s", flavor_id)
        ...
        os_utils.add_secgroup_to_instance(self.nova_client,
                                          instance.id,
                                          sg_id)
        self.__logger.info("Associating floating ip: '%s' to VM '%s' ",
                           floating_ip,
                           "cloudify_manager")
