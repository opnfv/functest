...
TEMPEST_LIST_FILE = REPO_PATH + \
                    functest_yaml.get("general").get("directories").get("dir_tempest_cases") \
                    + 'test_list.txt'
...
def configure_tempest(mode):
    ...
    if (mode == 'custom' or mode == 'smoke'):
        cmd += "testr list-tests smoke >" + TEMPEST_LIST_FILE
    elif (mode == 'full'):
        cmd += "testr list-tests >" + TEMPEST_LIST_FILE
    functest_utils.execute_command(cmd, logger)
