...
class FunctestUtilsTesting(unittest.TestCase):
        ...
        with mock.patch('functest.utils.functest_utils.get_db_url',
            ...
            self.assertFalse(functest_utils.
                             push_results_to_db(self.project, self.case_name,
                                                ...
                                                self.stop_date,
                                                self.criteria, self.details))
            mock_logger_error.assert_called_once_with(AnyStringWith("Pushing "
                                                                    "Result to"
                                                                    " DB"
                                                                    "(\S+\s*) "
                                                                    "failed:"))
    ...
    @mock.patch('functest.utils.functest_utils.get_functest_config')
    @mock.patch('functest.utils.functest_utils.logger.error')
    def test_get_deployment_dir_default(self, mock_logger_error,
                                        mock_get_functest_config):

        mock_get_functest_config.side_effect = self._get_functest_config
        deployment_dir = ('general.directories.dir_rally_inst' +
                          "/tempest/for-deployment-" +
                          'deployment_uuid')

        with mock.patch('functest.utils.functest_utils.subprocess.Popen') \
                as mock_subproc_open:
            mock_obj = mock.Mock()
            attrs = {'readline.return_value': 'deployment_uuid'}
            mock_obj.configure_mock(**attrs)

            mock_obj2 = mock.Mock()
            attrs = {'stdout': mock_obj}
            mock_obj2.configure_mock(**attrs)

            mock_subproc_open.return_value = mock_obj2

            self.assertEqual(functest_utils.get_deployment_dir(),
                             deployment_dir)

    @mock.patch('functest.utils.functest_utils.exit')
    @mock.patch('functest.utils.functest_utils.get_functest_config')
    @mock.patch('functest.utils.functest_utils.logger.error')
    def test_get_deployment_dir_failed(self, mock_logger_error,
                                       mock_get_functest_config,
                                       mock_exit):
        ...
        with mock.patch('functest.utils.functest_utils.subprocess.Popen') \
            ...
            mock_subproc_open.return_value = mock_obj2
            functest_utils.get_deployment_dir()
            mock_logger_error.assert_called_once_with("Rally deployment"
                                                      " not found.")
    ...
    def test_check_test_result_default(self):
        with mock.patch('functest.utils.functest_utils.get_criteria_by_test') \
                as mock_criteria:
            mock_criteria.return_value = self.criteria
            details = {'timestart': self.start_time,
                       'duration': round(self.stop_time - self.start_time, 1),
                       'status': 'PASS'}
            resp = functest_utils.check_test_result(self.testname, 2.0,
                                                    self.start_time,
                                                    self.stop_time)
            self.assertEqual(resp, ('PASS', details))

    def test_check_test_result_failed(self):
        with mock.patch('functest.utils.functest_utils.get_criteria_by_test') \
                as mock_criteria:
            mock_criteria.return_value = self.criteria
            details = {'timestart': self.start_time,
                       'duration': round(self.stop_time - self.start_time, 1),
                       'status': 'FAIL'}
            resp = functest_utils.check_test_result(self.testname, 3.0,
                                                    self.start_time,
                                                    self.stop_time)
            self.assertEqual(resp, ('FAIL', details))
