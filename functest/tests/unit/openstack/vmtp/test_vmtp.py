...
class VmtpInitTesting(unittest.TestCase):
    ...
    def _test_exc_init(self):
        ...
        for func in [
                'create_openstack_resources', 'generate_keys',
                'write_config', 'run_vmtp']:
...
class VmtpTesting(unittest.TestCase):
    ...
    def test_create_osresources1(self):
        self.testcase.create_openstack_resources()
        self.testcase.cloud.create_image.assert_called_once_with(
            'vmtp_{}'.format(self.testcase.guid),
            filename=getattr(config.CONF, 'vmtp_image'),
            meta=getattr(config.CONF, 'vmtp_extra_properties', None))

    def test_create_osresources2(self):
        # pylint: disable=bad-continuation
        with mock.patch.object(
                self.testcase.cloud, "create_image",
