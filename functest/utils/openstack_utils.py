...
def create_glance_image(glance_client, image_name, file_path, disk="qcow2", extra_properties={},
                 ...
                 if ( "hw_firmware_type" in extra_properties.keys() and
                      "short_id" in extra_properties.keys() ):
                       image = glance_client.images.create(name=image_name,
                                                ...
                                                short_id=extra_properties['short_id'])
            else:
                 image = glance_client.images.create(name=image_name,
                                                visibility=public,
                                                disk_format=disk,
...
def get_or_create_image(name, path, format, extra_properties):
    ...
    if image_id != '':
        ...
        image_id = create_glance_image(glance_client, name, path, format, extra_properties)
        if not image_id:
