...
class RallyOpenStackCI(RallyBase):
    """Rally OpenStack CI testcase implementation."""

    TESTS = ["neutron"]

    def __init__(self, **kwargs):
        """Initialize RallyOpenStackCI object."""
        if "case_name" not in kwargs:
            kwargs["case_name"] = "rally_openstack_ci"
        super(RallyOpenStackCI, self).__init__(**kwargs)
        self.test_name = 'all'
        self.task_yaml = None
    ...
    def prepare_run(self):
        ...
        with open(os.path.join(self.RALLY_DIR,
                               'openstack_ci.yaml'), 'r') as task_file:
