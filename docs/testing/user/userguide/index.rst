...
Reference tempeset configuration
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
When running manually is used, recommanded way to generate tempest configuration
file is:
...
then adjust the tempest.conf according to the SUT:
