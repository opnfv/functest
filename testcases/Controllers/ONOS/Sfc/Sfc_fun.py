...
class Sfc_fun:
    ...
    def createVm(self):
        ...
        for y in range(0, 3):
            ...
            url = 'http://'+self.nova_hostname+':8774/v2/'+self.tenant_id + \
                  '/servers'
    ...
    def checkVmState(self):
        ...
        for y in range(0, 3):
            url = 'http://' + \
                  self.nova_hostname+':8774/v2/servers/detail?name=vm'+str(y)
...
####################################################################
    ...
    def addFloatingIp(self):
        ...
        for ip_num in range(0, 2):
            ...
            data = json.dumps(Dicdata, indent=4)
            url = 'http://'+self.nova_hostname+':8774/v2/os-floating-ips'
            headers = {"Accept": "application/json",
                       "X-Auth-Token": self.token_id}
            response = requests.post(url, headers=headers,  data=data)
            if (response.status_code == 200):
            ...
            data = json.dumps(Dicdata1, indent=4)
            url = 'http://' + self.nova_hostname + ':8774/v2/servers/' + \
    ...
    def cleanup(self):
        ...
        print ("\n\t\tDeleting the VMs")
        for y in range(0, 3):
            url = 'http://'+self.nova_hostname+':8774/v2/servers/'+self.vm[y]
            headers = {"Accept": "application/json",
                       "X-Auth-Token": self.token_id}
            response = requests.delete(url, headers=headers)
            if (response.status_code == 204):
