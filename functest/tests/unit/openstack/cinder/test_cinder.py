...
class CinderTesting(unittest.TestCase):
    ...
    def test_write_data_exc1(self):
        self.cinder.ssh = mock.Mock()
        self.cinder.volume = mock.Mock(id='volume')
        self.cinder.sshvm = mock.Mock(id='sshvm')
        self.cinder.timeout = mock.Mock(id='volume_timeout')
        with mock.patch.object(self.cinder.cloud,
                               'attach_volume', side_effect=Exception), \
             mock.patch.object(self.cinder.cloud,
                               'detach_volume'), \
             mock.patch.object(self.cinder.ssh, 'exec_command',
                               return_value=ssh_exception):
            with self.assertRaises(Exception):
                self.cinder._write_data()

    def test_write_data(self):
        self.cinder.sshvm = munch.Munch()
        self.cinder.ssh = mock.Mock()
        with mock.patch.object(self.cinder.cloud, 'attach_volume',
                               return_value=True), \
             mock.patch.object(self.cinder.cloud, 'detach_volume',
                               return_value=True), \
             mock.patch.object(self.cinder.logger, 'debug',
                               return_value=True), \
             mock.patch.object(self.cinder.logger, 'info',
                               return_value=True), \
             mock.patch.object(self.cinder.ssh, 'exec_command',
                               return_value=True):
            self.cinder._write_data()
            #self.assertEqual(self.cinder._write_data(), testcase.TestCase.EX_OK)


    def test_read_data_exc(self):
        self.cinder.ssh2 = mock.Mock()
        with mock.patch.object(self.cinder.cloud,
                               'attach_volume', side_effect=Exception), \
             mock.patch.object(self.cinder.cloud,
                               'detach_volume', side_effect=Exception), \
             mock.patch.object(self.cinder.ssh2, 'exec_command',
                               return_value=ssh_exception):
            with self.assertRaises(Exception):
                self.cinder._read_data()

    def test_read_data(self):
        self.cinder.ssh2 = mock.Mock()
        with mock.patch.object(self.cinder.cloud,
                               'attach_volume', return_value=True), \
            mock.patch.object(self.cinder.cloud,
                              'detach_volume', return_value=True), \
            mock.patch.object(self.cinder.logger, 'debug',
                              return_value=True), \
            mock.patch.object(self.cinder.logger, 'info',
                              return_value=True), \
            mock.patch.object(self.cinder.ssh2, 'exec_command',
                              return_value=True):
            self.cinder._read_data()
             #self.assertEqual(self.cinder._read_data(), testcase.TestCase.EX_OK)
