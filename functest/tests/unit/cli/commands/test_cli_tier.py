...
class CliTierTesting(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with mock.patch('functest.cli.commands.cli_tier.tb') as mock_tb:
            self.cli_tier = cli_tier.CliTier()

    @mock.patch('functest.cli.commands.cli_tier.click.echo')
    def test_list(self, mock_click_echo):
        with mock.patch.object(self.cli_tier.tiers,'get_tiers', return_value = []) \
            as mock_tb_tiers:
            self.cli_tier.list()
    ...
    def test_show_missing_tier(self, mock_click_echo):
        with mock.patch.object(self.cli_tier.tiers, 'get_tier', return_value=None) \
                               as mock_tb_tier:
            self.cli_tier.show(self.tiername)
            tier_names = self.cli_tier.tiers.get_tier_names()
            mock_click_echo.assert_called_with("The tier with name '%s' does not exist. "
                       "Available tiers are:\n  %s\n" % (self.tiername, tier_names))

    @mock.patch('functest.cli.commands.cli_tier.click.echo')
    def test_gettests_missing_tier(self, mock_click_echo):
      with mock.patch.object(self.cli_tier.tiers, 'get_tier', return_value=None) as mock_tb_tier:
            self.cli_tier.show(self.tiername)
            tier_names = self.cli_tier.tiers.get_tier_names()
            mock_click_echo.assert_called_with("The tier with name '%s' does not exist. "
                       "Available tiers are:\n  %s\n" % (self.tiername, tier_names))

    @mock.patch('functest.cli.commands.cli_tier.os.path.isfile', return_value=False)
    @mock.patch('functest.cli.commands.cli_tier.click.echo')
    ...
    @mock.patch('functest.cli.commands.cli_tier.os.path.isfile', return_value=True)
