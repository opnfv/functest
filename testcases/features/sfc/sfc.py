...
# Download the image
    ...
    if not os.path.isfile(IMAGE_PATH):
        ...
        ft_utils.download_url("http://artifacts.opnfv.org/sfc/demo/sf_summit2016.qcow2","/home/opnfv/functest/data/")
    else:
...
### STARTING SECOND VM (server) ###
    ...
    if not os_utils.add_floating_ip(nova_client, instance_2.id, floatip_server):
        logger.error("Cannot associate floating IP to VM.")
        sys.exit(-1)

    #### CREATION OF THE 2 SF ####

    tacker_script = "/home/opnfv/repos/functest/testcases/features/sfc/" + TACKER_SCRIPT
        ...
        for instance in instances:
            if "server" not in instance.name:
                if "client" not in instance.name:
                    logger.debug("This is the instance name: %s " %instance.name)
                    floatip_dic = os_utils.create_floating_ip(neutron_client)
    ...
    try:
            ssh.connect(floatip_client, username="root",
                        password="opnfv", timeout=2)
            command = "nc -w 5 -zv " + floatip_server + " 22"
            (stdin, stdout, stderr) = ssh.exec_command(command)
    except:
    ...
    # WRITE THE CORRECT WAY TO DO loggerGING
    i = 0
    logger.info("First output: %s" % stdout)
    ...
    try:
            ssh.connect(floatip_client, username="root",
                        password="opnfv", timeout=2)
            command = "nc -w 5 -zv " + floatip_server + " 80"
            (stdin, stdout, stderr) = ssh.exec_command(command)
    except:
            logger.debug("Waiting for %s..." % floatip_client)
            time.sleep(6)
    ...
    tacker_classi = "/home/opnfv/repos/functest/testcases/features/sfc/" + TACKER_CHANGECLASSI
