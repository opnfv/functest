...
-----------------------------------
For the Triple-O based installers (like Apex) you can copy the
credentials file from the "stack" users home directory in
Instack/Undercloud VM to Jumphost::

  scp stack@<Specific IP Address>:overcloudrc .

  NOTE: Next section helps you finding out the IP address of the host.

Regardless of the installer you should make sure that there is not a
parameter "no_proxy" in the local copy of the credential file as it
might ruin your connectivity::
...
--------------------------
  #. It is a good practise to assign a precise container name through
     the **--name** option.

  #. For OPNFV official installers, it is recommended (although no
     longer mandatory) to provide environment variable for installer
     type::
  ...
  #. Functest may need to know the IP of the installer to retrieve
     ...
     The "INSTALLER_IP" environment variable should be set equal to the
     IP address of installer.

     For instance in case of Openstack's Triple-O based installer (like
     Apex), you can easily find the IP by executing the following
     command and note the returned IP address::
       ...
       execute the command "virsh list" to see which name is in use.

     The other installers need other means of finding the address.
  ...
  #. Credentials for accessing the Openstack.
     ...
     **-v** option. In the example we have local file by the name of
     "overcloudrc" and we are using that as an argument::
       ...
       The credential file need to exist in the Docker container under
       the path: '/home/opnfv/functest/conf/openstack.creds'.

     It is also possible to separately copy the credentials to inside
     the Functest container before running the tests, but it is not
     instructed here.
  ...
  #. Check the need for known SSH keys. In case of Openstack's Triple-O
     based installers (like Apex) the docker container needs to connect
     to the Instack/Undercloud VM, so it is then required that some
     known SSH keys are present in docker container. Since the Jumphost
     root SSH keys are already known, easiest way is to use those using
     the 'Bind mount' method. See below for sample parameter::

       -v /root/.ssh/id_rsa:/root/.ssh/id_rsa

       NOTE: This parameter might not be needed for other installers.
       NOTE: You need the "sudo" when creating the container to access
       root users ssh credentials even the docker command itself might
       not require that.

     See Apex command example in section `Apex Installer Tips`_ below.
...
Here is an example of the docker command invocation for an Apex
installed system, using latest Functest docker container, for
illustration purposes::

  sudo docker run -it --name "ApexFuncTstODL" \
  ...
  NOTE: You need the "sudo" to access root users ssh credentials even
  the docker itself does not nesessarily require that.
...
There are 3 ways to provide them to Functest:
  ...
  * using the -v option when creating the Docker container
  * create an empty file in '/home/opnfv/functest/conf/openstack.creds'
    and paste the credentials into it. (Consult your installer guide to
    know from where you can retrieve credential files, which are set-up
    in the Openstack installation of the SUT)
  * automatically retrieved using the following script::
      ...
      Note: If you omit the -d <full destination path> option in the
      command invocation, then the script will create the credential
      file with name 'opnfv-openrc.sh' in directory '/home/opnfv'. In
      that case, you need to copy/move the file into the correct target
      path: '/home/opnfv/functest/conf/openstack.creds'.

**Warning** If you are using the Joid installer, the 'fetch_os_cred.sh'
shell script **should not be used**. Use instead the **-v** option to
Bind Mount a suitably prepared local copy of the Openstack credentials
for usage by the Functest docker container.

Once the credentials are there, they should be sourced **before**
running the tests::
...
succeed:
  ...
  #. Execution of the Functest environment preparation inside the
     ...
     download some resources for some test cases. For example to
     install the Rally environment. This might not work properly if the
     Jumphost is connecting to internet through a http Proxy.
