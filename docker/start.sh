...
RALLY_REPO_DIR=$(cat $config_file | grep dir_repo_rally | awk 'END {print $NF}')
RELENG_REPO_DIR=$(cat $config_file | grep dir_repo_releng | awk 'END {print $NF}')
...
RALLY_VENV=$(cat $config_file | grep dir_rally_inst | awk 'END {print $NF}')
RALLY_COMMIT=$(cat $config_file | grep rally_stable_commit | awk 'END {print $NF}')
