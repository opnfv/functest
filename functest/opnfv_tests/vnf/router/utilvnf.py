...
class Utilvnf(object):
    ...
    def __init__(self):
        ...
        with open(os.environ["CONFIG_FUNCTEST_YAML"]) as file_fd:
            functest_yaml = yaml.safe_load(file_fd)
        file_fd.close()

        data_dir = functest_yaml.get("general").get("dir").get(
            "router_data") + "/"
        ...
        with open("/usr/local/lib/python2.7/dist-packages/" +
                  "functest/opnfv_tests/vnf/router/" +
                  "cloudify_vrouter.yaml") as file_fd:
    ...
    def get_mac_address(self, server_name, network_name):
        ...
        mac_address = \
            server.addresses[network_name][NOVA_CILENT_NETWORK_INFO_INDEX][
