...
class ConnectionCheckTesting(unittest.TestCase):
    ...
    @mock.patch('functest.opnfv_tests.openstack.snaps.connection_check.'
                'ConnectionCheck')
    def test_run_success(self, mock_logger_debug):
    ...
    @mock.patch('functest.opnfv_tests.openstack.snaps.connection_check.'
                'ConnectionCheck')
    def test_run_1_of_100_failures(self, mock_logger_debug):
        result = mock.MagicMock(name='unittest.TextTestResult')
        result.testsRun = 100
        result.failures = ['foo']
        result.errors = []
    ...
    @mock.patch('functest.opnfv_tests.openstack.snaps.connection_check.'
                'ConnectionCheck')
    def test_run_1_of_100_failures_within_criteria(self, mock_logger_debug):
