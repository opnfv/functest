...
def test_clearwater():
    ...
    dep_outputs = requests.get(api_url + "/deployments/" +
                               CONF.vims_clearwater_deployment_name + "/outputs")
    ...
    if dns_ip != "":
        ...
        script += ('rake test[' + CONF.vims_clearwater_inputs["public_domain"] +
                   '] SIGNUP_CODE="secret"')
...
def main():
    ...
    user_id = os_utils.create_user(
        keystone, CONF.vims_tenant_name, CONF.vims_tenant_name, None, tenant_id)
    if not user_id:
        logger.error("Error : Failed to create %s user" % CONF.vims_tenant_name)
    ...
    if not os_utils.update_sg_quota(neutron, tenant_id, 50, 100):
        step_failure(
            "init",
            "Failed to update security group quota for tenant " + CONF.vims_tenant_name)
    ...
    cfy.download_manager_blueprint(
        CONF.vims_cloudify_blueprint['url'], CONF.vims_cloudify_blueprint['branch'])
