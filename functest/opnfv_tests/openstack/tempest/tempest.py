...
class TempestCommon(testcase_base.TestcaseBase):
    ...
    def run(self):
        ...
        logger.info("Starting Tempest test suite: '%s'." % self.OPTION)
...
class TempestSmoke(TempestCommon):
    ...
    def run(self):
        ret_val = super(TempestSmoke, self).run()
        if ret_val != 0:
            return -1
...
class TempestFull(TempestCommon):
    ...
    def run(self):
        ret_val = super(TempestFull, self).run()
        if ret_val != 0:
            return -1
...
class TempestMultisite(TempestCommon):
    ...
    def run(self):
        ret_val = super(TempestMultisite, self).run()
        if ret_val != 0:
            return -1
