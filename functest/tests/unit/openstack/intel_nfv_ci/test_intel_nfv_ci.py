...
class IntelNfvCiTesting(unittest.TestCase):
    ...
    def test_is_numa_enabled(self):
        node = mock.Mock()
        file_obj = mock.Mock()
        file_obj.read.return_value = '2'
        node.exec_command.return_value = None, file_obj, None
    ...
    def test_is_numa_enabled_error(self):
        node = mock.Mock()
        file_obj = mock.Mock()
        file_obj.read.return_value = 'wrong'
        node.exec_command.return_value = None, file_obj, None
        result = self.intel.is_numa_enabled(node)
    ...
    def test_has_hyperthreading_enabled(self):
        node = mock.Mock()
        file_obj = mock.Mock()
        file_obj.read.return_value = '0-1\n'
        node.exec_command.return_value = None, file_obj, None
    ...
    def test_check_requirements_skip(self):
        ...
        self.assertEqual(self.intel.is_skipped, True)
