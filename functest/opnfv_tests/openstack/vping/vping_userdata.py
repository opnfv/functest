...
class VPingUserdata(vping_base.VPingBase):
    ...
    def run(self):
        """
        Sets up the OpenStack VM instance objects then executes the ping and validates.
        ...
        try:
            ...
            instance1_settings = VmInstanceSettings(name=self.vm1_name, flavor=self.flavor_name,
            ...
            self.vm1_creator = deploy_utils.create_vm_instance(
                self.os_creds, instance1_settings, self.image_creator.image_settings)

            userdata = _get_userdata(self.vm1_creator.get_port_ip(port1_settings.name))
            if userdata:
                # Creating Instance 2
                port2_settings = PortSettings(self.os_creds, name=self.vm2_name + '-vPingPort',
                                              network_name=self.network_creator.network_settings.name)
                instance2_settings = VmInstanceSettings(name=self.vm2_name, flavor=self.flavor_name,
                                                        vm_boot_timeout=self.vm_boot_timeout,
                ...
                self.vm2_creator = deploy_utils.create_vm_instance(
                    self.os_creds, instance2_settings, self.image_creator.image_settings, userdata=userdata)
            else:
    ...
    def _do_vping(self, vm_creator, test_ip):
        ...
        while True:
            time.sleep(1)
            p_console = vm_creator.vm.get_console_output()
            if "vPing OK" in p_console:
                ...
                if "request failed" in p_console:
                    self.logger.debug("It seems userdata is not supported in nova boot. Waiting a bit...")
                    tries += 1
                else:
