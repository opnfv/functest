...
class Runner(object):
    ...
    def update_config_file():
        """Update the config file according to some specified scenarios
        or for AARCH64"""
    ...
    def patch_file(patch_file_path):
        """Used to update config file"""

        LOGGER.debug('Updating file: %s', patch_file_path)
        with open(patch_file_path) as patch_f:
            patch_file = yaml.safe_load(patch_f)
        ...
        if updated:
            ...
            with open(CONFIG_FUNCTEST_PATH, "w") as conf_f:
                conf_f.write(yaml.dump(new_functest_yaml, default_style='"'))
