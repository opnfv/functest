...
class CloudifyImsPerf(cloudify_ims.CloudifyIms):
    ...
    def test_vnf(self):
        ...
        for ip in vcard_ips:
            ...
            response = json.loads(ChassisRestAPI.postOperation(cards_url,
                                                               ...
                                                               payload))
            count = 0
            while (int(ChassisRestAPI.getWithHeaders(response['url'],
                                                     api_key)['progress'])
                    != 100):
        ...
        result = resolver.query(self.vnf['inputs']['public_domain'])
        ...
        ssh = paramiko.SSHClient()
        ssh.set_missing_host_key_policy(paramiko.MissingHostKeyPolicy())
        ssh.connect(fip_load, username="ixload", password="ixia123")

        scp = SCPClient(ssh.get_transport())
        filename = '/mnt/ixload-share/*.*/Results/*/ \
                    PortEventViewer_VoIP_Error_Event_*.csv'
        # scp.get(filename, self.data_dir)
    ...
    def run(self, **kwargs):
        ...
        super(CloudifyImsPerf, self).run(**kwargs)
