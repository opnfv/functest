...
def configure_tempest(deployment_dir, IMAGE_ID=None, FLAVOR_ID=None):
    """
    Execute rally verify configuration of tempest.conf, adding/updating needed parameters
    """
    conf_verifier_result = configure_verifier_tempest(deployment_dir)
    if conf_verifier_result == releng_constants.EXIT_RUN_ERROR:
        return releng_constants.EXIT_RUN_ERROR
    else:
        configure_tempest_update_params(tempest_conf_file, IMAGE_ID=None, FLAVOR_ID=None)
        return releng_constants.EXIT_OK


def configure_tempest_update_params(tempest_conf_file, IMAGE_ID=None, FLAVOR_ID=None):
    """
...
def configure_verifier_tempest(deployment_dir):
    """
    Execute rally verify configuration for tempest generated by Rally
    """
