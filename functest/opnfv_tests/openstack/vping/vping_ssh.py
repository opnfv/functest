...
from snaps.openstack.create_security_group import SecurityGroupSettings, SecurityGroupRuleSettings, Direction, Protocol
...
class VPingSSH(vping_base.VPingBase):
    ...
    def __init__(self):
        ...
        self.kp_name = functest_utils.get_functest_config('vping.keypair_name') + self.guid
        self.kp_priv_file = functest_utils.get_functest_config('vping.keypair_priv_file')
        self.kp_pub_file = functest_utils.get_functest_config('vping.keypair_pub_file')
        self.router_name = functest_utils.get_functest_config('vping.vping_router_name') + self.guid
        self.sg_name = functest_utils.get_functest_config('vping.vping_sg_name') + self.guid
        self.sg_desc = functest_utils.get_functest_config('vping.vping_sg_descr')

    def run(self):
        """
        Sets up the OpenStack keypair, router, security group, and VM instance objects then validates the ping.
        ...
        try:
            ...
            self.kp_creator = deploy_utils.create_keypair(self.os_creds,
            ...
            self.router_creator = deploy_utils.create_router(self.os_creds, RouterSettings(
                self.os_creds, name=self.router_name, external_gateway='external',
                internal_subnets=[self.network_creator.network_settings.subnet_settings[0].name]))

            # Creating Instance 1
            port1_settings = PortSettings(self.os_creds, name=self.vm1_name + '-vPingPort',
                                          network_name=self.network_creator.network_settings.name)
            instance1_settings = VmInstanceSettings(
                name=self.vm1_name, flavor=self.flavor_name, vm_boot_timeout=self.vm_boot_timeout,
                vm_delete_timeout=self.vm_delete_timeout, ssh_connect_timeout=self.vm_ssh_connect_timeout,
                port_settings=[port1_settings])

            self.logger.info("Creating VM 1 instance with name: '%s'" % instance1_settings.name)
            self.vm1_creator = deploy_utils.create_vm_instance(self.os_creds, instance1_settings,
            ...
            port2_settings = PortSettings(self.os_creds, name=self.vm2_name + '-vPingPort',
                                          network_name=self.network_creator.network_settings.name)
            instance2_settings = VmInstanceSettings(
                ...
                floating_ip_settings=[FloatingIpSettings(
                    name=self.vm2_name + '-FIPName', port_name=port2_settings.name,
                    router_name=self.router_creator.router_settings.name)])

            self.logger.info("Creating VM 2 instance with name: '%s'" % instance2_settings.name)
            self.vm2_creator = deploy_utils.create_vm_instance(self.os_creds, instance2_settings,
                                                               self.image_creator.image_settings,
                                                               keypair_creator=self.kp_creator,
                                                               sg_names=[self.sg_creator.sec_grp_settings.name])
    ...
    def __create_security_group(self):
        ...
        """
        sg_rules = list()
        sg_rules.append(SecurityGroupRuleSettings(self.os_creds, sec_grp_name=self.sg_name, direction=Direction.ingress,
                                                  protocol=Protocol.icmp))
        sg_rules.append(SecurityGroupRuleSettings(self.os_creds, sec_grp_name=self.sg_name, direction=Direction.ingress,
                                                  protocol=Protocol.tcp, port_range_min=22, port_range_max=22))
        sg_rules.append(SecurityGroupRuleSettings(self.os_creds, sec_grp_name=self.sg_name, direction=Direction.egress,
                                                  protocol=Protocol.tcp, port_range_min=22, port_range_max=22))

        self.logger.info("Security group with name: '%s'" % self.sg_name)
        return deploy_utils.create_security_group(self.os_creds,
                                                  SecurityGroupSettings(self.os_creds, name=self.sg_name,
