...
class RunTestsTesting(unittest.TestCase):
    ...
    def test_source_rc_file_default(self, mock_logger_debug):
        with mock.patch('functest.ci.run_tests.os.path.isfile',
            ...
            run_tests.source_rc_file()
            mock_logger_debug.assert_any_call("Sourcing the "
                                              "OpenStack RC file...")
            mock_logger_debug.assert_any_call("OS_AUTH_URL:test_os_auth_url")
            mock_logger_debug.assert_any_call("OS_USERNAME:test_os_username")
            mock_logger_debug.assert_any_call("OS_TENANT_NAME:test_tenant")
            mock_logger_debug.assert_any_call("OS_PASSWORD:test_password")
