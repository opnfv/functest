...
class VPingBase(TestCase):
    ...
    def _execute(self):
        ...
        if self.vm1_creator.vm_active(
            ...
            self.success = self._do_vping(self.vm2_creator, test_ip)
        else:
            raise Exception('VMs never became active')

        if self.success == TestCase.EX_RUN_ERROR:
            return self.success

        self.stop_time = time.time()
        self.success = TestCase.EX_OK
        self.__parse_result(self.success, self.start_time, self.stop_time)
        return self.success
