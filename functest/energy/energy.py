...
class EnergyRecorder(object):
    ...
    def load_config():
        ...
        if EnergyRecorder.energy_recorder_api is None:
            assert env.Environment.get('NODE_NAME')
            assert env.Environment.get('ENERGY_RECORDER_API_URL')
            environment = env.Environment.get('NODE_NAME')
            energy_recorder_uri = env.Environment.get(
                'ENERGY_RECORDER_API_URL')

            # Creds
            creds_usr = env.Environment.get("ENERGY_RECORDER_API_USER")
            creds_pass = env.Environment.get("ENERGY_RECORDER_API_PASSWORD")
