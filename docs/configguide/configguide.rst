...
ways. The "functest env prepare" command will automatically look for
the Openstack credentials file "/home/opnfv/functest/conf/openstack.creds"
and retrieve it unless the file already exists. This environment
preparation step is described later in this document.
...
executing the following sequence
  ...
  #. # Create a container as described in next chapter
     # but do not "Bind Mount" the credentials
  #. # Log in to container and execute the following command.
     # Replace the IP with installer address after the "-a" parameter
...
HINT! In case of Triple-O installers you can find the installer
IP address by executing command and note the returned IP address::
...
----------------------------------
Once the credentials are there inside the container, they can be
