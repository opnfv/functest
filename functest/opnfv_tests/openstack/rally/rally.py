...
class RallyBase(testcase.TestCase):
    ...
    def _get_ext_net(self):
        """
        Return requested external network or the first
        one retrieved.
        """
        ext_nets = self.cloud.list_networks(filters={"router:external": True})
        if env.get('EXTERNAL_NETWORK'):
            name = env.get('EXTERNAL_NETWORK')
            for ext_net in ext_nets:
                if ext_net.name == name:
                    return ext_net
        return ext_nets[0] if ext_nets else None
