...
parser.add_argument('--config', action='store', dest='cfgfile', help='Config file', required=True)
...
def createfiles(host, port, user, user_key):
    ...
    connect = connect.connectionManager(host, port, user, user_key, localpath, remotepath, com)
...
def run_scanner(host, port, user, user_key, nodetype):
    ...
    if scantype == 'xccdf':
        ...
        com = '{0} oval eval --results {1}/{2} --report {1}/{3} {4}'.format(
                                                                    oscapbin,
                                                                    tmpdir.rstrip(),
                                                                    results,
                                                                    report,
...
def post_tasks(host, port, user, user_key, nodetype):
    ...
    connect = connect.connectionManager(host, port, user, user_key, dl_folder, reportfile,
					report, results)
