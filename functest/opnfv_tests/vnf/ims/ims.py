...
class ImsVnf(vnf_base.VnfOnBoardingBase):
    ...
    def __init__(self, project='functest', case='', repo='', cmd=''):
        ...
        self.case_dir = os.path.join(ft_constants.FUNCTEST_TEST_DIR,
                                     'vnf/ims/')
        self.data_dir = ft_constants.VIMS_DATA_DIR
        self.test_dir = ft_constants.VIMS_TEST_DIR

        self.orchestrator = dict(
            requirements=ft_constants.CFY_MANAGER_REQUIERMENTS,
            blueprint=ft_constants.CFY_MANAGER_BLUEPRINT,
            inputs=ft_constants.CFY_INPUTS
        )

        self.vnf = dict(
            blueprint=ft_constants.CW_BLUEPRINT,
            deployment_name=ft_constants.CW_DEPLOYMENT_NAME,
            inputs=ft_constants.CW_INPUTS,
            requirements=ft_constants.CW_REQUIERMENTS
        )
    ...
    def deploy_orchestrator(self, **kwargs):
        ...
        if 'os_image' in self.orchestrator.requirements.keys():
            image_id = os_utils.get_image_id(
                slef.glance_client, self.orchestrator.requirements['os_image'])
            if image_id == '':
    ...
    def deploy_vnf(self):
        ...
        if not flavor_id:
            self.logger.info("Available flavors are: ")
            self.pMsg(self.nova_client.flavor.list())
            error_code = self.step_failure("Failed to find required flavor" +
                                           " for this deployment")
        ...
        if 'os_image' in self.vnf.requirements.keys():
            image_id = os_utils.get_image_id(
                slef.glance_client, self.vnf.requirements['os_image'])
            if image_id == '':
                self.step_failure("Failed to find required OS image" +
                                  " for clearwater VMs")
        else:
    ...
    def test_vnf(self):
        script = "source " + self.data_dir + "venv_cloudify/bin/activate; "
        script += "cd " + self.data_dir + "; "
        ...
        if dns_ip != "":
            ...
            script += 'cd ' + self.data_dir + '; '
            script += ('rake test[' + self.inputs["public_domain"] +
                       '] SIGNUP_CODE="secret"')
        ...
        try:
            ...
            if 'ram_min' in requirements.keys():
                flavor_id = os_utils.get_flavor_id_by_ram_range(
                    nova, requirements['ram_min'], 7500)
